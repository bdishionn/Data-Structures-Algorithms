...
public class SpellCheckerTester {
	...
	public void testUsesTemplateProjectCorrectly()
		...
		assertTrue(
				"This project doesn't appear to be based on a copy of JavaCoreTemplate because the sbcccore library was not found in the lib directory.  See https://github.com/ProfessorStrenn/workspace-cs106#workspace-cs106 for help on using the JavaCoreTemplate.",
				count > 0);
		totalScore += 2;
	...
	private void verifyCoreVersion()
		...
		try {
			...
			var parts = ver.split("\\.");
			if (parseInt(parts[0]) < 1 || parseInt(parts[1]) < 0 || parseInt(parts[2]) < 9)
				throw new RangeException((short) 0, "sbcccore version is " + ver + ", but must be at least 1.0.9");

		} catch (RangeException | NoSuchMethodException e) {
			isZeroScore = true;
			scorePreamble = "RESUBMISSION REQUIRED :  ************  See JUnit test results for help  ************";
			fail("RESUBMISSION REQUIRED.  This project appears to be based on a previous semester's project template.  See https://github.com/ProfessorStrenn/workspace-cs106#to-use for help on using the JavaCoreTemplate.");
		}
	}
